...
!=========================== ABAQUS format user element subroutine ===================
    ...
    ! Write your code for a 2D element below
     ...
     1                                                       dN2dxi)
        ...
        B(1,4*NNODE_2+1:2*NNODE_tot-1:4) = dNdx(NNODE_2+1:NNODE,1)
        !Second line
        B(2,2:4*NNODE_2-2:4) = dNdx(1:NNODE_2,2)
        B(2,4*NNODE_2+2:2*NNODE_tot:4) = dNdx(NNODE_2+1:NNODE,2)
        !Third line
        B(3,1:4*NNODE_2-3:4) = dNdx(1:NNODE_2,2)
        B(3,4*NNODE_2+1:2*NNODE_tot-1:4) = dNdx(NNODE_2+1:NNODE,2)
        B(3,2:4*NNODE_2-2:4) = dNdx(1:NNODE_2,1)
        B(3,4*NNODE_2+2:2*NNODE_tot:4) = dNdx(NNODE_2+1:NNODE,1)
        ! Fourth line
    ...
    !1      *(2.d0*(sol(5)+dsol(5))-1.d0)
     ...
     3      matmul(D,B(1:9,1:2*NNODE_tot)))*w(kint)*det
