...
!          abq_UEL_2D_CB_integrationpoints           - defines integration points for 2D continuum elements
!          abq_UEL_2D_CB_shapefunctions              - defines shape functions for 2D continuum elements
!          abq_UEL_1D_CB_integrationpoints(n_points, n_nodes, xi, w)  = defines integration points for 1D line integral
!=========================== ABAQUS format user element subroutine ===================
    ...
    !
      ...
      double precision  ::  h, width, E, xnu                  ! beam thickness h, width w,  Youngs Modulus, Poissons ratio
      double precision  ::  L, sin1, sin2, cos1, cos2
      double precision  ::  slaves(2,4) 
      double precision  ::  delta1, delta2
      double precision  ::  ehat1(2), ehat2(2), strainhat(2)
      double precision  ::  Tmat(8,6), R(2,3), RBT(2,6)
      double precision  ::  T, shear, M


      n_points = 5

      call abq_UEL_1D_CB_integrationpoints(n_points, NNODE, xi, w)
      ...
      ! the coordinate difference of the master nodes (6th - 5th)
      delta1 = coords(1,2)-coords(1,1)
      delta2 = coords(2,2)-coords(2,1)
      
      ! length of the element
      L = sqrt(delta1**2.d0 +delta2**2.d0)
      
      !  small deformation assumptiom
      sin1 = delta2/L
      sin2 = delta2/L
      cos1 = delta1/L
      cos2 = delta1/L
      
      ! the coordinates of the slave nodes (1st to 4th)
      slaves(1,1) = coords(1,1) + h/2.d0*sin1
      slaves(2,1) = coords(2,1) - h/2.d0*cos1
      slaves(1,2) = coords(1,2) + h/2.d0*sin2
      slaves(2,2) = coords(2,2) - h/2.d0*cos2
      slaves(1,3) = coords(1,2) - h/2.d0*sin2
      slaves(2,3) = coords(2,2) + h/2.d0*cos2
      slaves(1,4) = coords(1,1) - h/2.d0*sin1
      slaves(2,4) = coords(2,1) + h/2.d0*cos1
      ...
      Tmat = 0.d0
      Tmat(1,1) = 1.d0
      Tmat(1,3) = coords(2,1) - slaves(2,1)
      Tmat(2,2) = 1.d0
      Tmat(2,3) = -(coords(1,1) - slaves(1,1))
      Tmat(3,4) = 1.d0
      Tmat(3,6) = coords(2,2) -slaves(2,2)
      Tmat(4,5) = 1.d0
      Tmat(4,6) = coords(1,2) - slaves(1,2)
      Tmat(5,4) = 1.d0
      Tmat(5,6) = coords(2,2) - slaves(2,3)
      Tmat(6,5) = 1.d0
      Tmat(6,6) = coords(1,2) - slaves(1,3)
      Tmat(7,1) = 1.d0
      Tmat(7,3) = coords(2,1) - slaves(2,4)
      Tmat(8,2) = 1.d0
      Tmat(8,3) = -(coords(1,1) - slaves(1,4))


      do kint = 1, n_points
        call abq_UEL_2D_CB_shapefunctions(xi(1:2,kint),NNODE,N,dNdxi)
        dxdxi = matmul(slaves(1:2,1:NNODE),dNdxi(1:NNODE,1:2))
        call abq_inverse_LU_CB(dxdxi,dxidx,2)
        ...
        B(3,1:2*NNODE-1:2) =  dNdx(1:NNODE,2)
        B(3,2:2*NNODE:2) =    dNdx(1:NNODE,1)

        ! Calculate the infinitesimal strains
        strain(1:3) = matmul(B(1:3,1:8),matmul(Tmat(1:8,1:6),U(1:6)))

        ! Calculate ehat
        ehat1(1:2) = dxdxi(1:2,1)
     1                  /sqrt(dxdxi(1,1)**2.d0+dxdxi(2,1)**2.d0)
        ehat2(1:2) = dxdxi(1:2,2)
     1                  /sqrt(dxdxi(1,2)**2.d0+dxdxi(2,2)**2.d0)
        !ehat1(1:2) = dxdxi(1:2,1)/sqrt(sum(dxdxi(1:2,1)**2.d0))
        !ehat2(1:2) = dxdxi(1:2,2)/sqrt(sum(dxdxi(1:2,2)**2.d0))
        ...
        R(2,3) = ehat1(1)*ehat2(2) + ehat2(1)*ehat1(2)

        ! strainhat = R * strai
        strainhat(1:2) = matmul(R(1:2,1:3),strain(1:3))

        ! Create stress hat
        stress(1:2) = matmul(D(1:2,1:2),strainhat(1:2))

        ! final calculation
        RBT(1:2,1:6) = matmul(R(1:2,1:3)
     1                ,matmul(B(1:3,1:8), Tmat(1:8,1:6)))
        RHS(1:6,1) = RHS(1:6,1)-width*L*h/2.d0
     1        *matmul(transpose(RBT(1:2,1:6)),stress(1:2))*w(kint)

        AMATRX(1:6,1:6) = AMATRX(1:6,1:6)
     1    +width*L*h/2.d0*matmul(transpose(RBT(1:2,1:6))
     2     ,matmul(D(1:2,1:2),RBT(1:2,1:6)))*w(kint)

        T = T + width*h/2.d0*stress(1)*w(kint)
        shear = shear + width*h/2.d0*stress(2)*w(kint)
        M = M + width*(h**2.d0)/4.d0*stress(1)*xi(2,kint)*w(kint)
      ...
      subroutine abq_UEL_2D_CB_integrationpoints(n_points, n_nodes, 
     1                                                       xi, w)
    ...
    !         Defines integration points and weights for 2D continuum elements
      ...
      end subroutine abq_UEL_2D_CB_integrationpoints




      subroutine abq_UEL_2D_CB_shapefunctions(xi,n_nodes,f,df)
      ...
      end subroutine abq_UEL_2D_CB_shapefunctions


      subroutine abq_UEL_1D_CB_integrationpoints(n_points, n_nodes, 
     1                                                        xi, w)
     ...
     1                    abq_UEL_1D_CB_integrationpoints'
      ...
      end subroutine abq_UEL_1D_CB_integrationpoints



      subroutine abq_facenodes_2D_CB(nelnodes,face,list,nfacenodes)
    ...
    !
      ...
      end subroutine abq_facenodes_2D_CB

      subroutine abq_inverse_LU_CB(Ain,A_inverse,n)  ! Compute the inverse of an arbitrary matrix by LU decomposition
      ...
      end subroutine abq_inverse_LU_CB