...
!=========================== ABAQUS format user element subroutine ===================
    ...
    !
    !       Variables tbar must be computed in this routine
    ...
    !
      ...
      double precision  ::  ebar1(2), ebar2(2), strainbar(2)
      double precision  ::  Tmat(8,6), R(2,3), RBT(2,6)
      double precision  ::  axialF, shearF, bendingM


      n_points = 5         !Trapezoidal integration
      ...
      do kint = 1, n_points
        call abq_UEL_2D_cb_shapefunctions(xi(1:2,kint),4,N,dNdxi)
        dxdxi = matmul(slaves(1:2,1:4),dNdxi(1:4,1:2))
        ...
        ! Calculate ebar
        ebar1(1:2) = dxdxi(1:2,1)/sqrt(sum(dxdxi(1:2,1)**2.d0))
        ebar2(1:2) = dxdxi(1:2,2)/sqrt(sum(dxdxi(1:2,2)**2.d0))

        ! Create R matrix
        R = 0.d0
        R(1,1) = ebar1(1)**2
        R(1,2) = ebar1(2)**2
        R(1,3) = ebar1(1)*ebar1(2)
        R(2,1) = 2*ebar1(1)*ebar2(1)
        R(2,2) = 2*ebar1(2)*ebar2(2)
        R(2,3) = ebar1(1)*ebar2(2) + ebar2(1)*ebar1(2)

        ! strainbar = R * strain
        strainbar(1:2) = matmul(R(1:2,1:3),strain(1:3))

        ! Create stress bar
        stress(1:2) = matmul(D(1:2,1:2),strainbar(1:2))

        ! final calculation
        RBT(1:2,1:6) = matmul(R(1:2,1:3),matmul(B(1:3,1:8)
     1                ,Tmat(1:8,1:6)))
     ...
     2     ,matmul(D(1:2,1:2),RBT(1:2,1:6)))*w(kint)
        ...
        axialF = axialF + width*h/2.d0*stress(1)*w(kint)
        
        shearF = shearF + width*h/2.d0*stress(2)*w(kint)
        
        bendingM = bendingM + width*(h**2.d0)/4.d0*stress(1)*xi(2,kint)*
     1                                                        w(kint)
      ...
      subroutine abq_UEL_2D_cb_integrationpoints(n_points, n_nodes, xi, 
     !                                                                w)
    ...
    !         Defines integration points and weights for 2D continuum elements
      ...
      subroutine abq_UEL_1D_cb_integrationpoints(n_points, n_nodes, xi, 
     !                                                                w)
